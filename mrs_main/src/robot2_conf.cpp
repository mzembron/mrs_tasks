...
{
  ros::init(argc, argv, "robot2_planner");
  float distance = 0;
  ...
  ros::Subscriber odomSubscriber = n.subscribe("/robot2_tf/odom", 1000, odomCallback);
  ros::Subscriber goalSubscriber = n.subscribe("/move_base_simple/goal", 1000, goalCallback);
  ros::Publisher velocityPublisher = n.advertise<geometry_msgs::Twist>("/robot2/cmd_vel", 1000); //maybe robot1/cmd_vel
  ros::Publisher distancePublisher = n.advertise<std_msgs::String>("mrs_robot2_distance_to_goal", 1000);
